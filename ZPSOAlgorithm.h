...
public:
    ...
    Benchmarks* fp;
    //double (*_fitnessFunction)(double* x); //粒子适应度函数
     ...
     *  maxSpeed：粒子运动最大速度，默认为1
     * 输出参数：
     *  ZPSO_Algorithm&：构建得到的PSO算法本身
    ***************************************************************/
    ZPSO_Algorithm(Benchmarks* fp1,
        ...
        double maxSpeed = 1)
    {
        ...
        for (int i = 0; i < _dimension; i++)
        {
            _positionMinValue[i] = positionMinValue[i];//来传入上下界数据。
        ...
        for (int i = 0; i < _particalCount; i++)
            _particalSet[i].initial(_dimension);  //初始化每一个粒子（开辟内存空间
        _globalBestPartical.initial(_dimension);  //初始化全局最优粒子
        //配置随机数种子
    ...
    ***************************************************************/
    void refresh(void)
    {
        ...
        {
            //循环遍历所有粒子，更新粒子适应度
            _particalSet[i]._fitness = this->fp->compute(_particalSet[i]._position);
        ...
        //更新全局最优粒子位置（先把所有粒子都遍历一遍，再更新最优粒子
    ...
    ***************************************************************/
    void randomlyInitial(void)
    {
        ...
        //更新粒子初代适应度值与最佳适应度值
        _particalSet[0]._fitness =this-> fp->compute(_particalSet[0]._position);
        ...
        for (int i = 1; i < _particalCount; i++)
        {
            ...
            //更新粒子初代适应度值与最佳适应度值
            _particalSet[i]._fitness = fp->compute(_particalSet[i]._position);
            _particalSet[i]._bestFitness = _particalSet[i]._fitness;
            if (_particalSet[i]._bestFitness > _globalBestPartical._bestFitness)
                globalBestParticalIndex = i;
        }
    ...
    ***************************************************************/
        ...
        double disturbanceVelocityCoe = 0.05)
    {
        ...
        for (int i = 0; i < times; i++) {
            if (50 == i || 100 == i || 1000 == i || 5000 == i || 10000 == i || 50000 == i || 100000 == i || 500000 == i) {
                cout << "第" << i << "次fitness:" << _globalBestPartical._bestFitness << endl;
            }
